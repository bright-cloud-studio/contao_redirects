...
class Redirect extends Contao_Backend
{
	...
	{
		Database::getInstance()->prepare("UPDATE tl_redirect SET published='1' WHERE IF(start = '', 0, CONVERT(start, UNSIGNED)) < ? AND IF(start = '', 0, CONVERT(start, UNSIGNED)) > 0 AND (IF(stop = '', 0, CONVERT(stop, UNSIGNED)) > ? OR IF(stop = '', 0, CONVERT(stop, UNSIGNED)) = 0)")->execute(time(), time());
		Database::getInstance()->prepare("UPDATE tl_redirect SET published='' WHERE IF(stop = '', 0, CONVERT(stop, UNSIGNED)) < ? AND IF(stop = '', 0, CONVERT(stop, UNSIGNED)) > 0")->execute(time());
	}
	...
	public function toggleVisibility($intId, $blnVisible, DataContainer $dc=null)
	{
		...
		if (is_array($GLOBALS['TL_DCA']['tl_redirect']['fields']['published']['save_callback']))
		{
